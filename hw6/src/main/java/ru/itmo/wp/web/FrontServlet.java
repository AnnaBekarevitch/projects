...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route,
        ...
        methods.put("before", null);
        methods.put(route.getAction(), null);
        methods.put("after", null);

        for (String method : methods.keySet()) {
            for (Class<?> clazz = pageClass; methods.get(method) == null && clazz != null; clazz = clazz.getSuperclass()) {
                try {
                    methods.put(method, clazz.getDeclaredMethod(method, HttpServletRequest.class, Map.class));
        ...
        try {
            ...
            throw new ServletException("Can't render template [pageClass=" + pageClass
                    + ", method=" + route.getAction() + "].", e);
        }
    }
