    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response)
        ...
        for (Map.Entry<String, String[]> e : request.getParameterMap().entrySet()) {
            if (e.getValue() != null) {
                ...
                view.put(e.getKey(), value);

            }
        }
        for (Class aClass : classes) {
            System.out.println(aClass.getName());
            if (aClass.getName().equals("java.util.Map")) {
                values.add(view);
            } else if (aClass.getName().equals("javax.servlet.http.HttpServletRequest")) {
                values.add(request);
            }
        }
        try {
            method.invoke(page, values.toArray());
        ...
        if (view.getOrDefault("lang", null) != null
                && view.get("lang").toString().length() == 2
                && view.get("lang").toString().matches("[a-z]+")) {
                session.setAttribute("lang", view.get("lang"));
        }
        String templateFileName = pageClass.getSimpleName() +
                (session.getAttribute("lang") != null ? "_" + session.getAttribute("lang") : "")
                + ".ftlh";
        Template template = newTemplate(templateFileName);
    ...
    private Template newTemplate(String templateName) throws ServletException {
        ...
        if (sourceConfiguration != null) {
            try {
                ...
                try {
                    template = sourceConfiguration.getTemplate(templateName.substring(0, templateName.length() - 8)+".ftlh");
                } catch (TemplateNotFoundException i) {
                    try {
                        template = sourceConfiguration.getTemplate("NotFoundPage.ftlh");
                    } catch (Exception e) {
                        // do nothing
                    }
                } catch (IOException e) {
